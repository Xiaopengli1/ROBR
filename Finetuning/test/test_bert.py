...
text = pd.read_csv("data/Industrial/raw_data_full.csv")
query = pd.read_csv("data/Industrial/Rewritedquery_cn.csv")
...
# for i in tqdm(range(5)):
    original_query = query.iloc[i]["原始query"]
    woman_query = query.iloc[i]["中年妇女"]
    man_query = query.iloc[i]["中年男性"]
    student_query = query.iloc[i]["学生"]
    old_query = query.iloc[i]["老年人"]
